...
public class InitUsersAndRoles {
    ...
    public void runAfterStartup() {
        ...
        if (userService.getAllRoles().size()<1) {
            ...
            roles.add(user);
            User newUser = new User(1L, "User", "UserLastName", 35, "user@mail.ru", "user", "user", roles);

            roles = new HashSet<>();
            roles.add(user);
            roles.add(admin);
            User newAdmin = new User(2L, "Admin", "AdminLastName", 55, "admin@mail.ru", "admin", "admin", roles);
